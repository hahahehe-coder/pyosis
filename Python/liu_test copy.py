...
#     """绘制桁架结构：节点大小表示约束类型，杆件粗细表示截面大小，颜色表示内力符号"""
...
#     # 添加杆件
#     for e, (i, j) in enumerate(edges):
#         G.add_edge(i, j, section=section_ids[e], force=N[e])
...
#     # 杆件绘制（粗细=截面面积比例，颜色=拉力(红)/压力(蓝)）
#     max_A = max([sections[s]['A'] for s in section_ids])
#     for e, (i, j) in enumerate(edges):
#         A = sections[section_ids[e]]['A']
#         width = 2 * (A / max_A)  # 归一化粗细
#         color = 'red' if N[e] > 0 else 'blue'  # 拉力红，压力蓝
#         nx.draw_networkx_edges(G, pos, edgelist=[(i, j)], width=width, edge_color=color, alpha=0.7)
    
#     # 标注荷载
#     for idx, (x, y) in enumerate(load_nodes):
#         fx, fy = loads[idx]
#         plt.arrow(x, y, fx/50, fy/50, head_width=0.3, head_length=0.3, fc='black', ec='black', label='荷载' if idx==0 else "")
    
#     # 图表设置
#     plt.xlabel('x坐标 (m)')
#     plt.ylabel('y坐标 (m)')
#     plt.title('优化后的桁架结构布置图')
#     plt.legend()
...
def plot_truss(nodes, edges, section_ids, N):
    ...
    plt.savefig('result_fig.png', dpi=600)
