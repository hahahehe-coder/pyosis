...
def osis_material_conc(nMat: int, strName: str, 
    ...
    eCode = eCode.upper()
    eGrade = eGrade.upper()
    return e.OSIS_Material(nMat, strName, "CONC", eCode, eGrade, nCrepShrk, dDmp)

def osis_material_steel(nMat: int, strName: str, eCode: Literal["JTGD64_2015"], eGrade: Literal["Q235", "Q345", "Q390", "Q420"], dDmp: float):
    """创建或修改钢材
    ...
    eCode = eCode.upper()
    eGrade = eGrade.upper()
    return e.OSIS_Material(nMat, strName, "STEEL", eCode, eGrade, -1, dDmp)

def osis_material_prestressed(nMat: int, strName: str, eCode: Literal["JTG3362_2018", "JTGD62_2004"], eGrade: str, dDmp: float):
    """创建或修改材料。
    ...
    eCode = eCode.upper()
    eGrade = eGrade.upper()
    return e.OSIS_Material(nMat, strName, "PRESTRESSED", eCode, eGrade, -1, dDmp)

# 这个函数后面还得继续改
def osis_material_custom(nMat: int, strName: str, dE: float=0, dG: float=0, dMu: float=0, dExpCoeff: float=0, dUnitWeight: float=0, dDensity: float=0, dDmp: float=0):
    """创建或修改材料。
    ...
    params = locals()
    e = OSISEngine.GetInstance()
    return e.OSIS_Material(nMat, strName, "CUSTOM", "", "", -1, -1, params)
